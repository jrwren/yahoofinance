...
namespace MaasOne.Net
{
 ...
 (!asyncArgs.TimedOut)
                        ...
                        try
                        {
                            using (System.IO.Stream stream = resp.GetResponseStream())
                            {
                                using (System.IO.MemoryStream ms = MaasOne.MyHelper.CopyStream(stream))
                                {
                                    ...
                                    {
                                        if (ex is ParseException) { dlException = ex; }
                                        else { dlException = new ParseException(ex); }
                                    }
                                }
                            }
                        }
        ...
        private WebException GetOrCreateWebException(Exception ex)
        {
            ...
            {
                return new System.Net.WebException("An Exception was throwed during download process. See InnerException for more details.", ex, System.Net.WebExceptionStatus.UnknownError, null);
            }
        }
